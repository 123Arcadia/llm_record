...
def init_model():
    ...
    # 多卡初始化，就爱你差可用gpu并设置dataParallel
...
def get_lr(it: int, all: int):
    ...
    # 余弦退火阶段
    # 学习衰减率
    decay_ratio = (it - warm_iters) / (lr_decay_iters - warm_iters)
...
def train_epoch(epoch: int):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if (step + 1 ) % args.accumuletion_steps == 0:
            # 取消梯度缩放，准备梯度裁剪
        ...
        if step % args.log_interval == 0:
            ...
            if args.swanlab:
                swanlab.log({
                    "loss": loss.item() * args.accmulation_steps,
...
if __name__ == '__main__':
    # ============命令行参数解析===============
    parser = argparse.ArgumentParser(description="Tiny-LLM Pretraing")

    # 基础训练参数
    parser.add_argument("--out_dir", type=str, default="base_model_215M", help="模型输出目录")
    parser.add_argument("--epoches", type=int, default=1, help="训练轮次")
    parser.add_argument("--batch_size", type=int, default=64, help="批次轮次")
    parser.add_argument("--learning_rate", type=float, default=2e-4, help="lr")
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu",
                        help="训练设备, 默认无gpu就cpu")
    parser.add_argument("--dtype", type=str, default="bfloat16", help="数据类型")
    # 实验跟踪和数据加载
    parser.add_argument("--user_swanlab", action="store_true", help="是否使用SwanLab进行实验跟踪")
    ...
    # 实验跟踪记录
    if args.num_workers:
        # 西安登录 swan.login(api_key)
        run = swanlab.init(
            project="happy-LLM",
            experiment_name="Pretrain-215-20250924_id0",
            config=args
        )
    ...
    train_loader = DataLoader(
        ...
        drop_last=False,  # 不丢弃最后一个不完整的bsz
        shuffle=True,
    ...
    # adam优化器
    optimizer = optim.Adam(model.parameters(), lr = args.learining_rate)

    # ==================== 开始训练 ====================
    # 计算每个epoch的迭代次数
    iter_per_epoch = len(train_ds)
