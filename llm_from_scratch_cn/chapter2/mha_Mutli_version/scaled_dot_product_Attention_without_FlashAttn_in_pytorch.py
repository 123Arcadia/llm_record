...
class Scaled_dot_Attention_Without_FlashAttn(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        if num_tokens <= self.context_length:
            ...
            mask = self.mask[:self.context_length, :self.context_length]
        context_vec = F.scaled_dot_product_attention(q, k, v, attn_mask=mask, dropout_p=dropout,
                                                     is_causal=False)
