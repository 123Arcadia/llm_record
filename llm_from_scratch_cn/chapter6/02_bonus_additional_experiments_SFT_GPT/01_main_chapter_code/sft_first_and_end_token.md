...
然而，正如在实验微调附加层中发现可以显着提高性能

(transformer块*12)

因此，我们还使**最后一个transformer块**和**将最后一个transformer块连接到输出层的最终LayerNorm模块**可训练
...
7. 使用 LoRA（低阶适应）与训练所有层（第 10 行与第 5 行）：保持模型冻结并添加可训练的 LoRA 层是训练所有模型参数的可行替代方案(请参阅附录 E)，甚至可以将性能提高 1%。 从使用 LoRA 时训练和验证准确率之间的差距降低约 1% 可以看出，这可能是由于过度拟合较少。此外，使用 LoRA 的速度也稍快一些，因为需要更新的参数较少。
