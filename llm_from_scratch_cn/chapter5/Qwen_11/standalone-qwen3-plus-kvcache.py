...
class GroupQueryAttention(nn.Module):
    ...
    def forward(self, x, mask, cos, sin,start_pos=0, cache=None):
        ...
        if cache is not None:
            prev_k, prev_v = cache
            k = torch.cat([prev_k, k], dim=2) # 在 num_tokens这一维度拼接
        ...
        return self.out_proj(context_vec), next_cache
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask, cos, sin, start_pos=0, cache=None):
        s = x
        x = self.norm1(x)
        x, next_cache = self.attn(x, mask, cos, sin, start_pos=start_pos, cache=cache)
        x += s
...
class Qwen3Model(nn.Module):
    ...
    def forward(self, in_dix, cache=None):
        ...
        if cache is not None:
            ...
            mask = torch.triu(torch.ones((pos_end, pos_end), device=x.device, dtype=torch.bool)
                              , diagonal=1)[pos_start:pos_end, :pos_end]
        else:
            pos_start = 0
            mask = torch.triu(torch.ones(num_tokens, num_tokens, dtype=torch.bool, device=x.device), diagonal=1)
        mask = mask[None, None, :, :] # shape=(1,1,num_tokens, num_tokens)

        for i, blk in enumerate(self.trf_blocks):
            blk_cache = cache.get(i) if cache else None
            x, new_blk_cache = blk(x, mask, self.cos, self.sin, start_pos=pos_start, cache=blk_cache)
            if cache is not None:
...
def generate_text_basic_stream(model, token_ids, max_new_tokens, eos_token_id=None):
    ...
    with torch.no_grad():
        ...
        for _ in range(max_new_tokens):
            ...
            if eos_token_id is not None and torch.all(eos_token_id == next_token): # next_token shape(bsz, 1)
                break
