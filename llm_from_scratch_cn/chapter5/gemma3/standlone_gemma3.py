...
class Gemma3Model(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        assert cfg["n_layers"] is not None and len(cfg["layer_types"]) == cfg["n_layers"]
        self.tok_emb = nn.Embedding(cfg["vocab_size"], cfg["emb_dim"], dtype=cfg["dtype"])
        self.blks = nn.ModuleList([
            TransformerBlock(cfg, attn_type=attn_type) for attn_type in cfg["layer_types"]
        ])
        self.final_norm = RMSNorm(cfg["emb_dim"], eps=1e-6)
        self.out_head = nn.Linear(cfg["emb_dim"], cfg["vocab_size"], bias=False, dtype=cfg["dtype"])
        self.cfg = cfg

        cos_local, sin_local = compute_rope_params(head_dim=cfg["head_dim"], theta_base=cfg["rope_local_base"],
                                                    context_length=cfg["context_length"], dtype=torch.float32)
        cos_global, sin_global = compute_rope_params(head_dim=cfg["head_dim"], theta_base=cfg["rope_base"],
                                                    context_length=cfg["context_length"], dtype=torch.float32)
    ...
    def _create_mask(self, seq_len, device):
        ...
        far_pat = torch.triu(ones, diagonal=self.cfg["sliding_window"]).T
        mask_local = mask_global | far_pat
        return mask_global, mask_local

    def forward(self, in_idx):
        b, seq_len = in_idx.shape
        x = self.tok_emb(in_idx) * (self.cfg["emb_dim"] ** 0.5)
        mask_global, mask_local = self._create_mask(seq_len, x.device)
        for blk in self.blks:
            x = blk(x, mask_global=mask_global, mask_local=mask_local,
...
if __name__ == '__main__':
    ...
    # print(model(torch.tensor([1, 2, 3]).unsqueeze(0)).shape)
