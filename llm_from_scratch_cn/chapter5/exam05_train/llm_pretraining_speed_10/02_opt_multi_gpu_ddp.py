...
def train_model_simple_with_timing(model, train_loader, val_loader, optimizer, device, num_epochs, eval_freq, eval_iter,
                                   start_context):
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        for inp_b, tgt_b in train_loader:
            optimizer.zero_grad()
            global_step += 1
            loss = calc_loss_batch(inp_b, tgt_b, model, device)
            loss.backward()
            optimizer.step()

            total_tokens += inp_b.numel()

            if global_step % eval_freq == 0:
                if use_cuda:
                    t_e.record()
                    torch.cuda.synchronize()
                    elapsed = t_s.elapsed_time(t_e) / 1000 # ms 到 s
                    t_s.record()
                else:
                    t1 = time.time()
                    elapsed = t1 - t0
                ...
                if rank == 0:
                    print(f"Ep {epoch + 1:0{len(str(global_step))}d}/{global_step}, "
...
def plot_losses(epochs_tensor, tokens_seen, train_losses, val_losses):
    ...
    ax1.plot(epochs_tensor, val_losses, label="Validation loss", linestyle="-.")
...
if __name__ == '__main__':
    ...
    train_losses, val_losses, tokens_seen, model = main(
        GPT_CONFIG_124M, OTHER_SETTINGS, rank, world_size
    )

    file_prefix = "02_opt_multi_gpu_ddp_"
    # 训练后
    if rank == 0:
        epochs_tensor = torch.linspace(0, OTHER_SETTINGS["num_epochs"], len(train_losses))
        plot_losses(epochs_tensor, tokens_seen, train_losses, val_losses)
        plt.savefig(file_prefix+"loss.jpg", dpi=300)
    ...
    print(f'{compiled=}')
    if compiled:
        torch.save(model._orig_mod.state_dict(), file_prefix+"model.pth")
    else:
        torch.save(model.state_dict(), file_prefix+"model.pth")

    model = GPTModel(GPT_CONFIG_124M)
    model.load_state_dict(torch.load(file_prefix+"model.pth", weights_only=True))
