...
from reasoning_from_scratch.ch06 import load_math_train, save_checkpoint
from reasoning_from_scratch.qwen3 import KVCache, Qwen3Model, QWEN_CONFIG_06_B

file_name = 'rlvr_grpo_orihginal_no_kl'
...
def get_args():
    parser = argparse.ArgumentParser(description=f"{file_name} --help")
    parser.add_argument("--steps", type=int, default=None, help="Number of training steps(default: len(data))")
    parser.add_argument("--num_rollouts", type=int, default=8, help="Number of rollouts per step")
    parser.add_argument("--max_new_tokens", type=int, default=512)
    parser.addd_argument("--temperature", type=float, default=0.8)
    parser.addd_argument("--top_p", type=float, default=0.9)
    parser.add_argument("--checkpoint_path", type=str, default=None)
    parser.add_argument("--eval_on_checkpoint", action="store_true", help="当保存model时运行math500的验证环节")
...
def append_step_metrics(current_step, steps, loss, reward_avg, tokens_per_sec, avg_response_len):
    METRICS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with METRICS_LOG_PATH.open("a", encoding="utf-8") as f:
        f.write(f"[Step {current_step}/{steps}] "
...
def append_eval_metrics(step_idx, acc, correct, total):
    METRICS_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with METRICS_LOG_PATH.open("a", encoding="utf-8") as f:
        f.write(f"[Eval step {step_idx}] math500_acc={acc:.4f} "
                f"({correct/total})\n")


def append_sample_logs(current_step, samples, max_samples=3):
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    with LOG_PATH.open("a", encoding="utf-8") as f:
        f.write(f"[Step {current_step}] sample outputs\n")
...
def train_rlvr_grpo(model,
    ...
    steps = len(math_data) if steps is None else steps
    ...
    try:
        for step in range(steps):
            ...
            current_step += 1
            example = math_data[step % len(math_data)] # 循环使用dataset
            stats = compute_grpo_loss(model, tokenizer, example=example, device=device,
            ...
            tokens_per_sec = step_tokens / step_time if step_tokens > 0 else 0.0
            append_step_metrics(
                current_step,
                steps,
                stats["loss"],
            ...
            if checkpoint_every and current_step %  checkpoint_every == 0:
                ...
                if eval_on_checkpoint and math500_eval_data:
                    ...
                    out_lath = (Path(checkpoint_dir / f"{SCRIPT_NAME}-step{current_step:05d}-math500.jsonl"))
                    ...
                    append_step_metrics(current_step, acc, num_correct, num_examples)
                    print(f"MATH-500 evaluate @ step {current_step}"
            ...
            print(f'[Step {current_step}/{steps}]'
                  ...
                  f'tok_sec={tokens_per_sec:.1f}'
                  f'avg_rep_len={avg_response_len:.1f}')

    except KeyboardInterrupt as e:
        ckp_path = save_checkpoint(model, checkpoint_dir, step=max(1, current_step), suffix="interrupt")
        print(f"\nKeyboardInterrupt. Saved checkpoint to {ckp_path}")
        return model
    return model
